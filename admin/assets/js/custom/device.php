	...
	datatable = $('.m_datatable').mDatatable({
		...
		columns: [{
			...
			template: function (row) {
				if(row.image!="") {
					return '<img src="../images/device/'+row.image+'" width="50" title="'+row.title+'">';
				} else {
					return '';
				}
			}
			...
			template: function (row, index, datatable) {
				...
				return '\
					...
					<a href="edit_device.php?id='+row.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Device Details">\
						<i class="la la-edit"></i>\
					</a>\
					<?php
					}
					if($prms_device_delete == '1') { ?>
					<a href="javascript:void(0);" onClick="removeRecord('+row.id+')" class="m_sweetalert_demo_9 m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Delete Device">\
						<i class="la la-trash"></i>\
					</a>\
					<?php
					} ?>
					<a href="javascript:void(0);" onClick="copyRecord('+row.id+')" class="m_sweetalert_demo_9 m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Copy device, It will copy with unpublished status">\
...
function removeRecord(id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/device.php?d_id="+id;
		}
	});
}
...
function copyRecord(id) {
	swal({
		title: "Are you sure to copy this device?",
		showCancelButton: true,
		type: "error",
		confirmButtonText: "OK"
	}).then(function (e) {
		if(e.value) {
			window.location.href = "controllers/device.php?c_id="+id;
		}
	});
}
...
function removeImage(id, r_img_id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/device.php?id="+id+"&r_img_id="+r_img_id;
		}
	});
}

function removeHeaderImage(id, r_img_id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/device.php?id="+id+"&r_h_img_id="+r_img_id;
		}
	});
}
