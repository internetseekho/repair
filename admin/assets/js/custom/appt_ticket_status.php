	...
	datatable = $('.m_datatable').mDatatable({
		...
		columns: [{
			...
			template: function (row, index, datatable) {
				...
				if(row.type!="fixed") {
					return '\
						<a href="edit_appt_ticket_status.php?id='+row.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Ticket Status">\
							<i class="la la-edit"></i>\
						</a>\
						<a href="javascript:void(0);" onClick="removeRecord('+row.id+')" class="m_sweetalert_demo_9 m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Delete Ticket Status">\
...
function removeRecord(id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/appt_ticket_status.php?d_id="+id;
		}
	});
}
...
function ajax_records_action(ids, type) {
	...
	$.ajax({
		url: "ajax/actions/appt_ticket_status.php",
		method: "POST",
		data: data,
		success: function (data) {
			...
			if(e.status == 'success') {
				...
				window.location.href = "appt_ticket_status.php";
...
function removeImage(id, r_img_id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/appt_ticket_status.php?id="+id+"&r_img_id="+r_img_id;
		}
	});
}
