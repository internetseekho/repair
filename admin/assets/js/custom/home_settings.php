			...
			template: function (row, index, datatable) {
				...
				if(row.type!="Inbuild") {
					return '\
						...
						<a href="edit_home_settings.php?id='+row.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit">\
							<i class="la la-edit"></i>\
						</a>\
						<?php
						//}
						//if($prms_brand_delete == '1') { ?>
						<a href="javascript:void(0);" onClick="removeRecord('+row.id+')" class="m_sweetalert_demo_9 m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Delete">\
...
function removeRecord(id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/home_settings.php?d_id="+id;
		}
	});
}
...
function ajax_records_action(ids, type) {
	...
	$.ajax({
		url: "ajax/actions/home_settings.php",
		method: "POST",
		data: data,
		success: function (data) {
			...
			if(e.status == 'success') {
				...
				window.location.href = "home_settings.php";
...
function removeImage(id, r_img_id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/home_settings.php?id="+id+"&r_img_id="+r_img_id;
		}
	});
}
...
var SnippetFormValidation = function() {
    var handleProfileFormSubmit = function() {
        $('#m_form_submit').click(function(e) {
			...
			if($('.description2').summernote('codeview.isActivated')) {
				$('.description2').summernote('codeview.deactivate');
			}
			return;
        });
    }
