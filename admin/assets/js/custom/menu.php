...
function DatatableDataListInit() {
		...
		columns: [{
			...
			template: function (row, index, datatable) {
				...
				return '\
					<?php
					if($prms_menu_edit == '1') { ?>
					<a href="edit_menu.php?id='+row.id+'&position=<?=$menu_position?>" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Menu Details">\
						<i class="la la-edit"></i>\
					</a>\
					<?php
					}
					if($prms_menu_delete == '1') { ?>
					<a href="javascript:void(0);" onClick="removeRecord('+row.id+')" class="m_sweetalert_demo_9 m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Delete Menu">\
	...
	$('#m_form_status, #m_form_type, #position').selectpicker();
}
...
function removeRecord(id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/menu.php?d_id="+id+"&position=<?=$menu_position?>";
		}
	});
}
...
function changePosition(location) {
	if(location) {
		window.location.href = "controllers/menu.php?position="+location;
	}
}
	...
	$.ajax({
		...
		success: function (data) {
			...
			if(e.status == 'success') {
				...
				window.location.href = "menu.php?position=<?=$menu_position?>";
...
function removeImage(id, r_img_id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/menu.php?id="+id+"&r_img_id="+r_img_id+"&position=<?=$menu_position?>";
		}
	});
}
