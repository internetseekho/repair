...
function DatatableDataListInit() {
	...
	datatable = $('.m_datatable').mDatatable({
		...
		columns: [{
			...
			template: function (row, index, datatable) {
				...
				return '\
					<a href="edit_email_template.php?id='+row.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Email Template Details">\
	...
	$('#m_form_tmpl_type').on('change', function () {
		datatable.search($(this).val(), 'filter_by');
	});
	...
	$('#m_form_status, #m_form_tmpl_type, #m_form_type').selectpicker();
}
...
function removeRecord(id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/email_template.php?d_id="+id;
		}
	});
}
...
function ajax_records_action(ids, type) {
	...
	$.ajax({
		url: "ajax/actions/email_template.php",
		method: "POST",
		data: data,
...
function removeImage(id, r_img_id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/email_template.php?id="+id+"&r_img_id="+r_img_id;
		}
	});
}
