	...
	datatable = $('.m_datatable').mDatatable({
		...
		columns: [{
			...
			template: function (row) {
				if(row.image!="") {
					return '<img src="../images/service/'+row.image+'" width="50" title="'+row.title+'">';
				} else {
					return '';
				}
			}
			...
			template: function (row, index, datatable) {
				...
				return '\
					<?php
					//if($prms_service_edit == '1') { ?>
					<a href="edit_service.php?id='+row.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Service Details">\
						<i class="la la-edit"></i>\
					</a>\
					<?php
					//}
					//if($prms_service_delete == '1') { ?>
					<a href="javascript:void(0);<?php /*?>controllers/service.php?d_id='+row.id+'<?php */?>" onClick="removeRecord('+row.id+')" class="m_sweetalert_demo_9 m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Delete Service">\
...
function removeRecord(id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/service.php?d_id="+id;
		}
	});
}
...
function removeImage(id, r_img_id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/service.php?id="+id+"&r_img_id="+r_img_id;
		}
	});
}
