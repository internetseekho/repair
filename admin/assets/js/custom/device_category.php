	...
	datatable = $('.m_datatable').mDatatable({
		...
		columns: [{
			...
			template: function (row) {
				if(row.image!="") {
					return '<img src="../images/categories/'+row.image+'" width="50" title="'+row.title+'">';
				} else {
					return '';
				}
			}
			...
			template: function (row, index, datatable) {
				...
				return '\
					...
					<a href="edit_category.php?id='+row.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Category Details">\
						<i class="la la-edit"></i>\
					</a>\
					<?php
					}
					if($prms_category_delete == '1') { ?>
					<a href="javascript:void(0);" onClick="removeRecord('+row.id+')" class="m_sweetalert_demo_9 m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Delete Category">\
...
function removeRecord(id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/device_categories.php?d_id="+id;
		}
	});
}
	...
	$.ajax({
		...
		success: function (data) {
			...
			if(e.status == 'success') {
				...
				window.location.href = "device_categories.php";
...
function removeImage(id, r_img_id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/device_categories.php?id="+id+"&r_img_id="+r_img_id;
		}
	});
}
