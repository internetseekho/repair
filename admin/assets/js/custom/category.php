	...
	datatable = $('.m_datatable').mDatatable({
		...
		columns: [{
			...
			template: function (row, index, datatable) {
				...
				return '\
					<a href="add-category.php?id='+row.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Category Details">\
...
function removeRecord(id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/blog.php?d_c_id="+id;
		}
	});
}
...
function ajax_records_action(ids, type) {
	...
	$.ajax({
		url: "ajax/actions/blog_category.php",
		method: "POST",
		data: data,
...
function removeImage(id, r_img_id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/blog.php?id="+id+"&r_img_id="+r_img_id;
		}
	});
}
