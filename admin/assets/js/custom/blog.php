	...
	datatable = $('.m_datatable').mDatatable({
		...
		columns: [{
			...
			template: function (row, index, datatable) {
				...
				return '\
					<a href="add-post.php?id='+row.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Blog Details">\
						<i class="la la-edit"></i>\
					</a>\
					<a href="javascript:void(0);" onClick="removeRecord('+row.id+')" class="m_sweetalert_demo_9 m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Delete Blog">\
...
function removeRecord(id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/blog.php?d_b_id="+id;
		}
	});
}
...
function removeImage(id, r_img_id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/blog.php?id="+id+"&r_b_img_id="+r_img_id;
		}
	});
}
