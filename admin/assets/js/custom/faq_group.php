	...
	datatable = $('.m_datatable').mDatatable({
		...
		columns: [{
			...
			template: function (row, index, datatable) {
				...
				return '\
					<a href="edit_faq_group.php?id='+row.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Faq Details">\
						<i class="la la-edit"></i>\
					</a>\
					<a href="javascript:void(0);" onClick="removeRecord('+row.id+')" class="m_sweetalert_demo_9 m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Delete Faq">\
...
function removeRecord(id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/faq_group.php?d_id="+id;
		}
	});
}
...
function ajax_records_action(ids, type) {
	...
	$.ajax({
		url: "ajax/actions/faq_group.php",
		method: "POST",
		data: data,
		success: function (data) {
			...
			if(e.status == 'success') {
				...
				window.location.href = "faqs_groups.php";
...
function removeImage(id, r_img_id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/faq_group.php?id="+id+"&r_img_id="+r_img_id;
		}
	});
}
