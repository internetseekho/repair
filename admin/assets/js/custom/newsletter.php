		...
		columns: [{
			...
			template: function (row, index, datatable) {
				...
				return '\
					...
					<a href="edit_newsletter.php?id='+row.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Newsletter Details">\
						<i class="la la-edit"></i>\
					</a>\
					<?php
					}
					if($prms_customer_delete == '1') { ?><?php */?>
...
function removeRecord(id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/newsletter.php?d_id="+id;
		}
	});
}
...
function ajax_records_action(ids, type) {
	...
	$.ajax({
		url: "ajax/actions/newsletter.php",
		method: "POST",
		data: data,
...
function removeImage(id, r_img_id) {
	swal({
		...
		if(e.value) {
			window.location.href = "controllers/newsletter.php?id="+id+"&r_img_id="+r_img_id;
		}
	});
}
