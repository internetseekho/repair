...
if(!empty($ids) && $type == "remove") {
	...
	foreach($ids as $id_k=>$id_v) {
		...
		$category_q=mysqli_query($db,'SELECT image FROM brand WHERE id="'.$id_v.'"');
		$brand_data=mysqli_fetch_assoc($category_q);
		if($brand_data['image']!="")
	...
	foreach($ids as $id_k=>$id_v) {
		$removed_idd[] = $id_v;
		$query=mysqli_query($db,'UPDATE brand SET published=1 WHERE id="'.$id_v.'"');
	}
	...
	foreach($ids as $id_k=>$id_v) {
		$removed_idd[] = $id_v;
		$query=mysqli_query($db,'UPDATE brand SET published=0 WHERE id="'.$id_v.'"');
	}
