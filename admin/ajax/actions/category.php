...
if(!empty($ids) && $type == "remove") {
	...
	foreach($ids as $id_k=>$id_v) {
		$removed_idd[] = $id_v;
		$query=mysqli_query($db,'UPDATE categories SET published=1 WHERE id="'.$id_v.'"');
	}
	...
	foreach($ids as $id_k=>$id_v) {
		$removed_idd[] = $id_v;
		$query=mysqli_query($db,'UPDATE categories SET published=0 WHERE id="'.$id_v.'"');
	}
	...
	foreach($ids as $key => $ordering_data) {
		...
		if($exp_ordering_data[0]>0 && $exp_ordering_data[1]>0) {
			$query = mysqli_query($db,"UPDATE categories SET ordering='".$exp_ordering_data[1]."' WHERE id='".$exp_ordering_data[0]."'");
		}
	}
	if($query=="1") {
