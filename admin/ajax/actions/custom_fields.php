...
if(!empty($ids) && $type == "remove") {
	...
	foreach($ids as $id_k=>$id_v) {
		...
		if($no_of_fields>0){
			while($row_cus_fld = mysqli_fetch_assoc($exe_cus_fld)){
				$sql_cus_opt = "delete from custom_options where field_id = '".$id_v."'";
				$exe_cus_opt = mysqli_query($db,$sql_cus_opt);
			}
		   $query= mysqli_query($db,"delete from custom_fields where custom_group_id in (".$id_v.")");
		}
		$query= mysqli_query($db,"delete from custom_group where id in (".$id_v.")");
	}
	...
	foreach($ids as $id_k=>$id_v) {
		$removed_idd[] = $id_v;
		$query=mysqli_query($db,'UPDATE custom_group SET status=1 WHERE id="'.$id_v.'"');
	}
	...
	foreach($ids as $id_k=>$id_v) {
		$removed_idd[] = $id_v;
		$query=mysqli_query($db,'UPDATE custom_group SET status=0 WHERE id="'.$id_v.'"');
	}
	...
	foreach($ids as $key => $ordering_data) {
		...
		if($exp_ordering_data[0]>0 && $exp_ordering_data[1]>0) {
			$query = mysqli_query($db,"UPDATE brand SET ordering='".$exp_ordering_data[1]."' WHERE id='".$exp_ordering_data[0]."'");
		}
	}
	if($query=="1") {
