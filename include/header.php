...
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js" integrity="sha384-Tc5IQib027qvyjSMfHjOMaLkfuWVxZxUPnCJA7l2mCWNIpG9mGCD8wGNIcPD7Txa" crossorigin="anonymous"></script>
	...
	<?php /*?><link rel="stylesheet" type="text/css" href="<?=SITE_URL?>include/rs-plugin/css/settings.css" media="screen" />
		...
		<div id="top-bar" class="d-none d-md-block">
			...
			<div class="container clearfix">
				...
				<div class="col_half nobottommargin">
					<p class="nobottommargin"><strong><i class="icon-call"></i></strong> <a href="tel:<?=$site_phone?>"><?=$site_phone?></a> | <strong><i class="icon-line-clock"></i></strong> <?=$header_service_hours_text?></p>
				</div>
								...
								<li>
									...
									if(count($topright_menu_data['submenu'])>0) {
										...
										if(isset($_SESSION['user_id']) && $_SESSION['user_id']>0) { ?>
											...
											<li><a href="<?=SITE_URL?>controllers/logout.php"><i class="fa fa-sign-out"></i> <?=$logout_btn_text?></a></li>
										<?php
										} else { ?>
											<li><a href="<?=$login_link?>"><i class="fa fa-sign-in"></i> <?=$signin_btn_text?></a></li>
							...
							<li>
								...
								<ul>
									...
									if(isset($_SESSION['user_id']) && $_SESSION['user_id']>0) { ?>
										<li><a href="<?=SITE_URL?>account?orders"><?=$orders_btn_text?></a></li>
										<li><a href="<?=SITE_URL?>account?profile"><?=$profile_btn_text?></a></li>
										<li><a href="<?=SITE_URL?>controllers/logout.php"><i class="icon-line2-logout"></i> <?=$logout_btn_text?></a></li>
									<?php
									} else { ?>
										<li><a href="<?=$login_link?>"><i class="icon-signin"></i> <?=$signin_btn_text?></a></li>
										<li><a href="<?=$signup_link?>"><i class="icon-user-plus"></i> <?=$signup_btn_text?></a></li>
									<?php
									} ?>
								</ul>
							</li>
				...
				<div class="container clearfix">
					<div class="row">
						<div id="top-header-logo" class="col-9 col-md-3">
							...
							============================================= -->
							<div id="logo">
								<a href="<?=SITE_URL?>" class="standard-logo" data-dark-logo="<?=$logo_url?>"><img src="<?=$logo_url?>" alt="<?=SITE_NAME?>"></a>
								<a href="<?=SITE_URL?>" class="retina-logo" data-dark-logo="<?=$logo_url?>"><img src="<?=$logo_url?>" alt="<?=SITE_NAME?>"></a>
							</div><!-- #logo end -->
						</div>
							...
							<nav id="primary-menu">
								...
								if($is_act_header_menu == '1') { ?>
									...
									foreach($header_menu_list as $header_menu_data) {
										...
										<?php /*?><li class="<?=(($header_menu_data['brand_id']>0 && $header_menu_data['devices_in_submenu']=='1') || $header_menu_data['cats_in_menu']=='1' || $header_menu_data['brands_in_menu']=='1'?' mega-menu':'').($header_menu_data['id']==$active_page_data['menu_id'] || $header_menu_data['id']==$active_page_data['parent_menu_id'] || $header_menu_data['brand_id']==$device_single_data_resp['device_single_data']['brand_id']?' current':'')?>"><?php */?>
										<li class="<?=(($header_menu_data['brand_id']>0 && $header_menu_data['devices_in_submenu']=='1') || $header_menu_data['cats_in_menu']=='1' || $header_menu_data['brands_in_menu']=='1'?' ':'').((isset($active_page_data['menu_id']) && $header_menu_data['id']==$active_page_data['menu_id']) || (isset($active_page_data['parent_menu_id']) && $header_menu_data['id']==$active_page_data['parent_menu_id']) || (isset($device_single_data_resp['device_single_data']['brand_id']) && $header_menu_data['brand_id']==$device_single_data_resp['device_single_data']['brand_id'])?' current':'')?>">
										<a href="<?=$menu_url?>" class="<?=$header_menu_data['css_menu_class']?>" <?=$is_open_new_window?>><div><?=$header_menu_data['menu_name'].$menu_fa_icon?></div></a>
											<?php
											...
											if(count($header_menu_data['submenu'])>0) {
												...
												<ul class="sub-menu">
													...
													foreach($header_submenu_list as $header_submenu_data) {
														...
														<li <?=(isset($active_page_data['menu_id']) && $header_submenu_data['id']==$active_page_data['menu_id']?'class="current"':'')?>>
															<a href="<?=$s_menu_url?>" class="<?=$header_submenu_data['css_menu_class']?>" <?=$s_is_open_new_window?>><?=$header_submenu_data['menu_name'].$submenu_fa_icon?></a>
														</li>
													<?php
													} ?>
												</ul>
									...
									if(!empty($h_menu_list)) {
										foreach($h_menu_list as $pl_key=>$pl_data) {
											$sef_url = $pl_data['url']; ?>
											<li class="<?php if($sef_url==$url_first_param){echo 'active';} echo ' '.($pl_data['menu_align']==''||$pl_data['menu_align']=='left'?'left':'right')?>">
												<a <?=($pl_data['is_open_new_window']=='1' ?'target="_blank"':'')?> class="<?=($pl_data['css_menu_class']!='' ?$pl_data['css_menu_class']:'item')?>" href="<?=($pl_data['is_custom_url']=='1'?$sef_url:SITE_URL.$sef_url)?>"><?=$pl_data['menu_name']?>
													<?php
								...
								<div id="top-search">
									...
									<form action="<?=SITE_URL?>search" method="get">
										<input type="text" name="search" id="srch_list_of_model" class="form-control srch_list_of_model" value="" placeholder="<?=$searchbox_placeholder_text?>">
									</form>
								</div><!-- #top-search end -->
		  ...
		  <div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-body">
						<form class="mb-0" method="post" action="<?=SITE_URL?>controllers/newsletter.php" id="newsletter">
							<div class="row">
								<div class="col-md-5">
									<img class="privy-element privy-image-element lazy" src="<?=SITE_URL?>images/newsletter_main_img.webp">
								</div>
								<div class="col-md-7">
									...
									<div class="borderbox lightpink newsletter_fields">
										<div class="form-group">
											<input type="text" name="first_name" id="first_name" placeholder="First Name" class="sm-form-control" value="<?=$user_first_name?>" required />
										</div>
										<div class="form-group">
											<input type="text" name="last_name" id="last_name" placeholder="Last Name" class="sm-form-control" value="<?=$user_last_name?>" required />
										</div>
										<div class="form-group">
											<input type="email" name="email" id="email" placeholder="Email" class="sm-form-control" value="<?=$user_email?>" required />
										</div>
