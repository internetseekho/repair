<div id="device-prop-area" class="col-xs-12 <?php if($select_fields_as_box_or_add_remove_button == "box"){echo 'fields-as-boxes';}else{echo 'fields-as-ar-brn';}?> fields-multiple-steps">
	<?php
	...
	while($row_cus_fld = mysqli_fetch_assoc($exe_cus_fld)) {
		...
		<div class="cod-xs-12 model-details-panel" id="fld-<?=$fld_id?>" data-row_type="<?=$input_type?>" data-required="<?=$row_cus_fld['is_required']?>">
			...
			<div class="col-md-12 acc_content clearfix nopadding my-4">
				<?php
				$add_or_sub = '+';
				$price_type = '1';
				...
				elseif($input_type=="checkbox") {
					...
					$no_of_dd_options = mysqli_num_rows($exe_cus_opt);
					if($no_of_dd_options>0) {
						echo '<div class="checkboxes col-xs-12 btn-group-toggle radio_select_repair_buttons" data-input-type="'.$input_type.'" data-toggle="buttons">';
						while($row_cus_opt = mysqli_fetch_assoc($exe_cus_opt)) {
							$checked = '';
							...
							<label style="width:180px;height:240px;margin:5px;" for="<?=$chk_lab_slug.'-'.$row_cus_opt['id']?>" id="label-<?=$chk_lab_slug.'-'.$row_cus_opt['id']?>" class="col-md-3 col-xs-6 font-body rounded-0 radio_btn <?php if($select_fields_as_box_or_add_remove_button == "box"){echo ' btn btn-outline-dark';} if($row_cus_opt['icon']!=""){echo ' image_have';} if($checked == 'checked'){echo ' active';}?>">
...
<div class="">
	...
	<div class="col-md-6" id="next-button-id">
...
$(".radio_btn").on('click',function(e){
	//e.preventDefault();
//	$('body').scrollTo('#next-button-id2');
	//document.getElementById('next-button-id2').scrollIntoView()
	console.log('testing');
