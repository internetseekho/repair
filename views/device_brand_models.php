...
$long_description = $brand_data['long_description'];
...
//echo $category_id.":".$brand_id;exit;
...
<section id="model">
		...
		<ul class="clients-grid grid-4 bottommargin-sm clearfix">
			...
			foreach($device_data_list as $device_data) { ?>
				<li>
					...
					<a href="<?=SITE_URL.$brand_data['sef_url'].'/'.$device_data['sef_url']?>" style="opacity: 1;">

					<div style="background-image: url('<?=SITE_URL.'images/device/'.$device_data['device_img'];?>');height: 150px;background-repeat: no-repeat;background-size: 914px 150px;">
					<?php
					if($device_data['device_img']) {
						...
						<!-- <img loading="lazy" src="<?=$md_img_path?>" alt="<?=$device_data['title']?>"> -->
					<?php
					} ?>
	...
	if($model_num_of_rows>0) { ?>
		...
		if($device_id>0) {
			...
			$sub_title = $device_data['device_sub_title'];
			$description = $device_data['description'];
			$long_description = $device_data['long_description'];
		}
